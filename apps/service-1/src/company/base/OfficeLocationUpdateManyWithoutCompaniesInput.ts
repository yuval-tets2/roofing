/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { OfficeLocationWhereUniqueInput } from "../../officeLocation/base/OfficeLocationWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class OfficeLocationUpdateManyWithoutCompaniesInput {
  @Field(() => [OfficeLocationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OfficeLocationWhereUniqueInput],
  })
  connect?: Array<OfficeLocationWhereUniqueInput>;

  @Field(() => [OfficeLocationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OfficeLocationWhereUniqueInput],
  })
  disconnect?: Array<OfficeLocationWhereUniqueInput>;

  @Field(() => [OfficeLocationWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OfficeLocationWhereUniqueInput],
  })
  set?: Array<OfficeLocationWhereUniqueInput>;
}

export { OfficeLocationUpdateManyWithoutCompaniesInput as OfficeLocationUpdateManyWithoutCompaniesInput };
