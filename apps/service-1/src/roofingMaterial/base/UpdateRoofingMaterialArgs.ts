/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { RoofingMaterialWhereUniqueInput } from "./RoofingMaterialWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { RoofingMaterialUpdateInput } from "./RoofingMaterialUpdateInput";

@ArgsType()
class UpdateRoofingMaterialArgs {
  @ApiProperty({
    required: true,
    type: () => RoofingMaterialWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => RoofingMaterialWhereUniqueInput)
  @Field(() => RoofingMaterialWhereUniqueInput, { nullable: false })
  where!: RoofingMaterialWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => RoofingMaterialUpdateInput,
  })
  @ValidateNested()
  @Type(() => RoofingMaterialUpdateInput)
  @Field(() => RoofingMaterialUpdateInput, { nullable: false })
  data!: RoofingMaterialUpdateInput;
}

export { UpdateRoofingMaterialArgs as UpdateRoofingMaterialArgs };
